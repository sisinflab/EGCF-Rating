...
class RoGERModel(torch.nn.Module, ABC):
    ...
    def add_graph_loss(self, curr_raw_adj, gu, gi):
        ...
        graph_loss += -self.beta * torch.mm(ones_vec.unsqueeze(0).to(self.device), torch.log(torch.mm(curr_raw_adj.to(self.device), ones_vec.unsqueeze(-1).to(self.device)) + 1e-12)).squeeze() / curr_raw_adj.shape[-1]
        # SPARSITY LOSS
